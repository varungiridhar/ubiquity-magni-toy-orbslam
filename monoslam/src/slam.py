...
class SLAM(object):
  def __init__(self, W, H, K):
    ...
    self.W, self.H = W, H
    self.K = K
    self.disp2d = Display2D(self.W, self.H)
    self.disp3d = Display3D()
    self.subcriber = rospy.Subscriber("/camera/front/color/image_raw/compressed", CompressedImage, self.callback, queue_size=1)
    self.i = 0
  def callback(self, ros_data):
      ...
      if self.i % 8 == 0:
        ...
        frame = cv2.imdecode(np.fromstring(ros_data.data, np.uint8), cv2.IMREAD_COLOR)
        frame = cv2.resize(frame, (self.W, self.H))     
        self.process_frame(frame, None if gt_pose is None else np.linalg.inv(gt_pose[i]))
 
        # 3-D display
        #print(self.disp3d)
        if self.disp3d is not None:
          self.disp3d.paint(self.mapp)
        data_3Dmap_pub = rospy.Publisher('/data/3Dmap/json', String, queue_size=10)
        data_3Dmap_pub.publish(str(self.mapp.serialize()))

        if self.disp2d is not None:
          img = self.mapp.frames[-1].annotate(frame)
...
def main(args):
  ...
  rospy.spin()
  data = slam.mapp.serialize()
  print(str(data))
  with open('src/monoslam/src/data.json', 'w') as outfile:
